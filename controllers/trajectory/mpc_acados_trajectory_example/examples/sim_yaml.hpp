...
#ifndef MPC_ACADOS_TRAJECTORY_EXAMPLES_SIM_YAML_HPP_
#define MPC_ACADOS_TRAJECTORY_EXAMPLES_SIM_YAML_HPP_
...
inline void readSimYaml(const std::string& file_path, YamlSimConfig& config) {
  ...
  if (controller_cfg) {
    ...
    if (controller_cfg["ocp_json_file_path"]) {
      config.ocp_json_file_path = controller_cfg["ocp_json_file_path"].as<std::string>();
    }
  }
  ...
  for (std::size_t index = 0; index < sim_cfg["waypoints"].size(); ++index) {
    config.waypoints.push_back(detail::waypointFromNode(
        sim_cfg["waypoints"][index], "sim_config.waypoints[" + std::to_string(index) + "]"));
  }
}

}  // namespace mpc_acados_trajectory_examples

#endif  // MPC_ACADOS_TRAJECTORY_EXAMPLES_SIM_YAML_HPP_
