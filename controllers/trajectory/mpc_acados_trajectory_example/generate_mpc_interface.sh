...
# mpc_acados_trajectory Python CLI. Works from any CWD.
#
# Requirements: ``mpc_acados_core`` and ``mpc_acados_trajectory`` must be
...
python3 -m mpc_acados_trajectory.acados_solver \
  -c "${SCRIPT_DIR}/solver_definition_mpc_trajectory.yaml" \
  -o "${SCRIPT_DIR}"
