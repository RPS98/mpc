...
Trajectory-tracking MPC controller variant. Uses 9 online parameters per stage:
`desired_position[3]` + `desired_orientation[4]` + `desired_velocity[3]` +
`desired_acceleration[3]`.

See [`../../README.md`](../../README.md) for the overall architecture, build
and install instructions. The Python distribution is `mpc_acados_trajectory`
...
| `include/mpc_acados_trajectory/*.hpp`       | generated     | C++ headers (datatypes + wrapper).         |
| `src/*.cpp`                                 | generated     | C++ sources.                               |
| `mpc_acados_trajectory/*.py`                | generated     | Python package.                            |
