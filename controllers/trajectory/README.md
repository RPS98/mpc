...
│   ├── solver_definition_mpc_trajectory.yaml    (input to the Python CLI)
...
exactly what the `mpc_acados_trajectory.acados_solver` Python CLI does to
...
1. **acados** — build from source with `ACADOS_SOURCE_DIR` exported:
   ...
   ```
2. **mpc_acados_core** and **mpc_acados_trajectory** — install or put on `PYTHONPATH`:
   ```bash
   ...
   export PYTHONPATH=/path/to/mpc:/path/to/mpc/controllers/trajectory:$PYTHONPATH
   ```
...
python3 -m mpc_acados_trajectory.acados_solver \
  -c .../solver_definition_mpc_trajectory.yaml \
  -o .../mpc_acados_trajectory_example
```

The CLI:

1. Locates the `cpp_interface/` template inside the installed
   `mpc_acados_trajectory` package (no relative paths).
2. Copies it to `mpc_acados_trajectory_example/mpc_interface/`.
...
python3 -m mpc_acados_trajectory.acados_solver \
  -c /abs/path/.../solver_definition_mpc_trajectory.yaml

# Via the thin shell wrapper (output goes next to the script)
bash /abs/path/.../mpc_acados_trajectory_example/generate_mpc_interface.sh
```
...
1. Make sure `mpc_acados_core` and `mpc_acados_trajectory` are importable
   (editable install or `PYTHONPATH`).
2. Copy `solver_definition_mpc_trajectory.yaml` into your repo (or author
   your own with the same schema).
3. Run the CLI from anywhere:
   ```bash
   python3 -m mpc_acados_trajectory.acados_solver \
     -c /path/to/your/solver_definition_mpc_trajectory.yaml \
...
| `mpc_acados_trajectory/acados_solver.py`                              | yes          | Python CLI entry point (`-m mpc_acados_trajectory.acados_solver`). |
| `mpc_acados_trajectory_example/generate_mpc_interface.sh`             | yes          | Thin wrapper around the Python CLI.          |
