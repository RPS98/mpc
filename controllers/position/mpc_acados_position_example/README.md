# mpc_acados_position_example

Minimal consumer project showing how to integrate the `mpc_acados_position`
controller into a downstream repository.
...
├── solver_definition_mpc_position.yaml    ← acados solver config (input to generate)
...
3. `mpc_acados_position` installed (`pip install -e
   /path/to/controllers/position`). If you cloned this repo, run first
   `controllers/position/generate_datatypes.sh` to generate the library.
...
1. Copies `../mpc_acados_position/cpp_interface/` into `mpc_interface/`.
...
Re-run this script whenever `solver_definition_mpc_position.yaml` or the
...
```
build/mpc_interface/libmpc_acados_position.so
...
2. Edit `generate_mpc_interface.sh` and set `LIB_CPP_DIR` to your local
   `mpc_acados_position/cpp_interface/` path.
...
5. From your top-level `CMakeLists.txt`, either `add_subdirectory(<this
   ...
   the library target `mpc_acados_position::mpc_acados_position`.
